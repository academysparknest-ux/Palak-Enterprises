...
import { Plus, Upload, Loader2, ChevronLeft, ChevronRight, Search } from 'lucide-react';
import {
  getIdCardPersons,
  createIdCardPerson,
  updateIdCardPerson,
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-1.5 rounded-md border border-slate-200 px-3.5 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50"
          >
  ...
  async function handleSubmit(data: Record<string, string>) {
    ...
    try {
      if (person) {
        ...
        await createIdCardPerson({ ...(data as Partial<IdCardPerson>), project_id: projectId } as Parameters<
          typeof createIdCardPerson
        >[0]);
      }
      onSaved();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4">
      <div className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white p-6 shadow-xl">
        <h2 className="text-lg font-semibold text-slate-900">{person ? 'Edit Student' : 'Add Student'}</h2>

        {person && (
          <div className="mt-3">
            <PhotoUpload personId={person.id} photoPath={photoPath} onChange={setPhotoPath} />
          </div>
        )}
        {!person && (
          <p className="mt-2 text-xs text-slate-400">Photo can be added after the student is created.</p>
        )}
