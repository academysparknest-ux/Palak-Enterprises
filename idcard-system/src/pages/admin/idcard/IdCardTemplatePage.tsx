...
import { TemplateEditor, DEFAULT_TEMPLATE_LAYOUT } from '../../../components/idcard/TemplateEditor';
...
export default function IdCardTemplatePage() {
  ...
  async function handleSave() {
    ...
    try {
      if (template) {
        const updated = await updateIdCardTemplate(template.id, { name, layout });
        setTemplate(updated);
      } else {
  ...
  return (
    <div>
      ...
      <div className="mt-5">
        <TemplateEditor layout={layout} onChange={setLayout} widthMm={85.6} heightMm={54} />
      </div>
    </div>
  );
}
