...
async function loopReset() {
  ...
  console.log('2. Deleting order_files in small batches...');
  let totalFiles = 0;
  while (true) {
    const res = await client.query('DELETE FROM public.order_files WHERE id IN (SELECT id FROM public.order_files LIMIT 5);');
    if (res.rowCount === 0) break;
    totalFiles += res.rowCount;
    console.log(`   -> Deleted ${totalFiles} order_files so far...`);
  }

  console.log('3. Deleting order_items in small batches...');
  let totalItems = 0;
  while (true) {
    const res = await client.query('DELETE FROM public.order_items WHERE id IN (SELECT id FROM public.order_items LIMIT 5);');
    if (res.rowCount === 0) break;
    totalItems += res.rowCount;
    console.log(`   -> Deleted ${totalItems} order_items so far...`);
  }

  console.log('4. Deleting print_jobs in small batches...');
  let totalJobs = 0;
  while (true) {
    const res = await client.query('DELETE FROM public.print_jobs WHERE id IN (SELECT id FROM public.print_jobs LIMIT 5);');
    if (res.rowCount === 0) break;
    totalJobs += res.rowCount;
    console.log(`   -> Deleted ${totalJobs} print_jobs so far...`);
  }

  console.log('5. Deleting orders in small batches...');
  let totalOrders = 0;
  while (true) {
    const res = await client.query('DELETE FROM public.orders WHERE id IN (SELECT id FROM public.orders LIMIT 5);');
    if (res.rowCount === 0) break;
    totalOrders += res.rowCount;
    console.log(`   -> Deleted ${totalOrders} orders so far...`);
  }

  console.log('6. Deleting status_history in small batches...');
  let totalStatus = 0;
  while (true) {
    const res = await client.query("DELETE FROM public.status_history WHERE id IN (SELECT id FROM public.status_history WHERE entity_type = 'order' LIMIT 20);");
    if (res.rowCount === 0) break;
    totalStatus += res.rowCount;
    console.log(`   -> Deleted ${totalStatus} status_history records so far...`);
  }

  console.log('7. Deleting customer-documents order storage objects...');
  ...
  console.log('\n================================================================');
  console.log('8. RUNNING POST-RESET DATABASE INTEGRITY AUDIT');
  console.log('================================================================');
