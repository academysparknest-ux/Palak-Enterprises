...
async function resetCascade() {
  ...
  console.log('1. Executing DELETE FROM public.orders (Cascades to order_files, order_items, print_jobs, and unlinks invoices)...');
  const oRes = await client.query('DELETE FROM public.orders;');
  console.log(`[PASS] Deleted ${oRes.rowCount} orders!`);

  console.log('2. Deleting order status_history...');
  const shRes = await client.query("DELETE FROM public.status_history WHERE entity_type = 'order';");
  console.log(`[PASS] Deleted ${shRes.rowCount} status history records!`);

  console.log('3. Deleting storage objects in customer-documents...');
  ...
  console.log('\n================================================================');
  console.log('4. RUNNING POST-RESET DATABASE INTEGRITY AUDIT');
  console.log('================================================================');
...
resetCascade().catch(console.error);
