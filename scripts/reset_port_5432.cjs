...
async function main() {
  console.log('Connecting to PostgreSQL Session Pooler on port 5432...');
  ...
  console.log('1. Safely unlinking invoices (SET order_id = NULL)...');
  const inv = await client.query('UPDATE public.invoices SET order_id = NULL WHERE order_id IS NOT NULL;');
  console.log(`[PASS] Unlinked ${inv.rowCount} invoices referencing orders (all 24 financial records preserved)`);
  ...
  console.log(`[PASS] Deleted ${status.rowCount} order status_history records`);
  ...
  console.log(`[PASS] Deleted ${storage.rowCount} storage objects`);
