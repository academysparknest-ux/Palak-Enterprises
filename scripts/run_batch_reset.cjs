...
async function batchReset() {
  ...
  console.log('Step 1: Unlinking invoices...');
  const inv = await client.query('UPDATE public.invoices SET order_id = NULL WHERE order_id IS NOT NULL;');
  console.log(`  -> Unlinked ${inv.rowCount} invoices`);

  console.log('Step 2: Deleting order_files...');
  const of = await client.query('DELETE FROM public.order_files WHERE id IN (SELECT id FROM public.order_files LIMIT 500);');
  console.log(`  -> Deleted ${of.rowCount} order_files`);

  console.log('Step 3: Deleting order_items...');
  const oi = await client.query('DELETE FROM public.order_items WHERE id IN (SELECT id FROM public.order_items LIMIT 500);');
  console.log(`  -> Deleted ${oi.rowCount} order_items`);

  console.log('Step 4: Deleting print_jobs...');
  const pj = await client.query('DELETE FROM public.print_jobs WHERE id IN (SELECT id FROM public.print_jobs LIMIT 500);');
  console.log(`  -> Deleted ${pj.rowCount} print_jobs`);

  console.log('Step 5: Deleting orders...');
  const o = await client.query('DELETE FROM public.orders WHERE id IN (SELECT id FROM public.orders LIMIT 500);');
  console.log(`  -> Deleted ${o.rowCount} orders`);

  console.log('Step 6: Deleting order status history...');
  const sh = await client.query("DELETE FROM public.status_history WHERE entity_type = 'order';");
  console.log(`  -> Deleted ${sh.rowCount} order status_history records`);

  console.log('Step 7: Deleting customer documents in storage...');
  const sto = await client.query("DELETE FROM storage.objects WHERE bucket_id = 'customer-documents' AND (name LIKE 'orders/%' OR name LIKE 'PHOTO-%');");
  console.log(`  -> Deleted ${sto.rowCount} storage objects`);

  console.log('\nStep 8: Auditing final counts...');
...
batchReset().catch(console.error);
