...
function createPgClient() {
  ...
  c.on('error', (err) => {
    // Prevent unhandled error crashes on connection close
  });
  return c;
}

async function execQuery(label, sql) {
  for (let i = 1; i <= 5; i++) {
    ...
    try {
      ...
      console.log(`  [Retry ${i}/5] ${label} -> ${err.message}`);
      if (i === 5) throw err;
...
async function run() {
  console.log('================================================================');
  console.log('PALAK PRINTING PRESS — ORDER RESET EXECUTION');
  console.log('================================================================\n');

  console.log('1. Deleting order_files...');
  await execQuery('Delete order_files', 'DELETE FROM public.order_files;');

  console.log('2. Deleting order_items...');
  await execQuery('Delete order_items', 'DELETE FROM public.order_items;');

  console.log('3. Deleting print_jobs...');
  await execQuery('Delete print_jobs', 'DELETE FROM public.print_jobs;');

  console.log('4. Unlinking invoices referencing orders...');
  await execQuery('Unlink invoices', 'UPDATE public.invoices SET order_id = NULL WHERE order_id IS NOT NULL;');

  console.log('5. Deleting parent orders...');
  await execQuery('Delete orders', 'DELETE FROM public.orders;');

  console.log('6. Deleting order status history...');
  await execQuery('Delete order status_history', "DELETE FROM public.status_history WHERE entity_type = 'order';");

  console.log('7. Deleting order storage objects...');
  await execQuery('Delete customer-documents storage objects', "DELETE FROM storage.objects WHERE bucket_id = 'customer-documents' AND (name LIKE 'orders/%' OR name LIKE 'PHOTO-%');");

  console.log('\n================================================================');
  console.log('8. Running complete post-cleanup database integrity audit...');
  console.log('================================================================');

  const auditRes = await execQuery('Post-cleanup audit', `
    SELECT 
...
run().catch(err => {
  console.error('[FAIL] Run failed:', err);
  process.exit(1);
});
