...
export const InstantOnlineServicesSection: React.FC<InstantOnlineServicesSectionProps> = ({
  ...
  return (
    <section className={cn("space-y-6 sm:space-y-8", className)}>
      {/* Quick Service Section Header */}
      <div className="rounded-3xl border border-line bg-[#123B70] text-white p-6 sm:p-8 lg:p-10 shadow-raised relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl space-y-4">
          ...
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight text-white leading-tight">
            {currentLang === "hi"
              ? "दस्तावेज़ भेजें • विकल्प चुनें • दुकान से प्राप्त करें"
              : "Send your document, choose your printing requirements, and collect it from the shop when ready."}
          </h2>

          <p className="text-xs sm:text-sm text-slate-200 font-medium leading-relaxed max-w-2xl">
            {currentLang === "hi"
              ? "दुकान आने से पहले अपनी फाइल अपलोड करें। हम आपकी आवश्यकता अनुसार प्रिंट तैयार रखेंगे ताकि आपको लाइन में न लगना पड़े।"
              : "Need something printed? Send it to Palak. We'll prepare it. You collect it."}
          </p>

          {/* 4-Step Visual (01 Send -> 02 Choose -> 03 Pay -> 04 Collect) */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 pt-2">
            <div className="rounded-xl bg-white/10 p-3 border border-white/15 backdrop-blur-xs space-y-1">
              <div className="flex items-center gap-1.5 text-amber-300 font-black text-xs">
                <span className="h-5 w-5 rounded-md bg-amber-400 text-slate-950 font-black flex items-center justify-center text-[10px]">01</span>
                <span>{currentLang === "hi" ? "भेजें (Send)" : "01 — Send"}</span>
              </div>
              <p className="text-[11px] text-slate-200">
                {currentLang === "hi" ? "अपना दस्तावेज़ अपलोड करें (PDF, JPG, PNG, DOCX)" : "Upload your document (PDF, JPG, PNG, DOCX)."}
              </p>
            </div>

            <div className="rounded-xl bg-white/10 p-3 border border-white/15 backdrop-blur-xs space-y-1">
              <div className="flex items-center gap-1.5 text-amber-300 font-black text-xs">
                <span className="h-5 w-5 rounded-md bg-amber-400 text-slate-950 font-black flex items-center justify-center text-[10px]">02</span>
                <span>{currentLang === "hi" ? "चुनें (Choose)" : "02 — Choose"}</span>
              </div>
              <p className="text-[11px] text-slate-200">
                {currentLang === "hi" ? "कागज़, साइज़, कलर, कॉपी व बाइंडिंग चुनें" : "Select paper, size, colour, copies & binding."}
              </p>
            </div>
            ...
            <div className="rounded-xl bg-white/10 p-3 border border-white/15 backdrop-blur-xs space-y-1">
              <div className="flex items-center gap-1.5 text-emerald-300 font-black text-xs">
                <span className="h-5 w-5 rounded-md bg-emerald-400 text-slate-950 font-black flex items-center justify-center text-[10px]">04</span>
                <span>{currentLang === "hi" ? "प्राप्त करें (Collect)" : "04 — Collect"}</span>
              </div>
              <p className="text-[11px] text-slate-200">
                {currentLang === "hi" ? "हम आपका ऑर्डर तैयार रखेंगे" : "We'll prepare your order for pickup."}
              </p>
            </div>
          </div>
          ...
          <div className="pt-2 flex flex-wrap items-center gap-3">
            <Link
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-400 hover:bg-amber-300 px-5 sm:px-6 py-2.5 sm:py-3 text-xs sm:text-sm font-black text-slate-950 shadow-md transition-transform hover:scale-105 cursor-pointer"
            >
              <span>{currentLang === "hi" ? "📄 दस्तावेज़ भेजें (Send Document)" : "📄 Send Document"}</span>
