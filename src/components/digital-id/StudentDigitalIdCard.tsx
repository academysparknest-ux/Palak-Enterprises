...
export const StudentDigitalIdCard: React.FC<StudentDigitalIdCardProps> = ({
  ...
  if (fields.dateOfBirth) {
    detailItems.push({
      label: "Date of Birth",
      value: formatDisplayDate(fields.dateOfBirth),
      icon: Calendar,
    });
  }
  ...
  if (fields.academicYear || organization.academicYear) {
    detailItems.push({
      label: "Session",
      value: fields.academicYear || organization.academicYear || "",
      icon: Calendar,
    });
  }
  ...
  return (
    ...
    >
      {/* Glossy / Holographic Top Ambient Glow */}
      <div className="absolute top-0 inset-x-0 h-40 bg-gradient-to-br from-[#123B70] via-[#1E4D8C] to-[#0A254A] opacity-95" />
        ...
        <div className="flex items-center justify-between gap-3 mb-2">
          ...
          <div className="flex-1 text-center pr-1">
            ...
            <div className="flex items-center justify-center gap-1.5 mt-0.5">
              <span className="inline-block px-2 py-0.5 rounded-full bg-white/20 backdrop-blur-sm text-[10px] font-extrabold uppercase tracking-widest text-amber-200 border border-white/15">
                STUDENT DIGITAL ID
              </span>
            </div>
          </div>
          ...
          <div className="h-10 w-10 rounded-full bg-gradient-to-tr from-amber-300 via-teal-200 to-indigo-300 p-0.5 shadow-md shrink-0 opacity-90 animate-pulse">
            <div className="h-full w-full rounded-full bg-[#123B70]/80 backdrop-blur-sm flex items-center justify-center">
      ...
      <div className="relative z-10 px-5 sm:px-6 pt-1 pb-4">
        <div className="bg-white rounded-2xl p-4 sm:p-5 shadow-lg border border-slate-100/90 text-center">
          {/* Profile Photo with Frame and Hologram ring */}
          <div className="relative mx-auto w-28 h-36 sm:w-32 sm:h-40 mb-3.5 group">
            <div className="absolute -inset-1 rounded-2xl bg-gradient-to-tr from-[#123B70] via-cyan-500 to-amber-400 opacity-70 blur-xs" />
            ...
            {/* Official Verification Check Stamp */}
            {status === "active" && (
          ...
          {/* Student ID Pill */}
          <div className="inline-flex items-center gap-1.5 px-3 py-1 mt-1.5 rounded-lg bg-blue-50 border border-blue-200/60 text-[#123B70] text-xs font-black tracking-wider">
            <QrCode className="h-3.5 w-3.5 text-[#123B70]" />
            <span>ID: {id}</span>
          </div>
