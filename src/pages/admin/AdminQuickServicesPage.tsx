...
export const AdminQuickServicesPage: React.FC = () => {
  ...
  const saveConfig = async () => {
    ...
    setSaving(true);
    try {
      const success = await updatePrintPricingConfig(config);
      ...
      addToast({ type: 'error', title: 'Failed to save configuration', message: formatAdminErrorMessage(err, 'Unable to update quick services pricing. Please check permissions.') });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="pl-8 pr-3 block w-full rounded-xl border border-slate-200 bg-white py-2 text-xs font-bold text-slate-900 focus:ring-2 focus:ring-[#123B70] focus:border-[#123B70] transition-colors"
        />
        {suffix && (
          <span className="absolute inset-y-0 right-0 pr-3 flex items-center text-[10px] text-slate-400 font-semibold pointer-events-none">
            {suffix}
          </span>
        )}
      </div>
  ...
  return (
    <div className="space-y-6">
        ...
        actions={
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 bg-white text-slate-700 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors text-xs font-semibold shadow-xs cursor-pointer disabled:opacity-50"
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition-colors text-xs font-bold shadow-xs cursor-pointer disabled:opacity-50"
            >
      ...
      <div className="rounded-2xl border border-blue-200/80 bg-blue-50/60 p-4 text-xs text-[#123B70] flex items-start gap-3 shadow-xs">
        <AlertCircle className="w-4 h-4 text-[#123B70] shrink-0 mt-0.5" />
        <div className="space-y-0.5">
          <p className="font-bold">Live Synchronized Online Service Engine</p>
          <p className="text-slate-600 text-[11px]">
            Every change made here updates the customer-facing forms (e.g. Document Printing calculator, Photo Sheets, Visiting Cards) in real-time. Decimal values and finishing toggles are strictly validated.
          </p>
        </div>
      </div>
          ...
          {SERVICES.map((service) => {
            ...
            return (
              <div
                key={service.id}
                className={cn(
                  "bg-white rounded-2xl border transition-all duration-200 overflow-hidden shadow-xs",
              ...
              >
                ...
                <div
                  ...
                  className="p-4 sm:p-5 flex items-center justify-between cursor-pointer select-none bg-white hover:bg-slate-50/60 transition-colors"
                >
                  ...
                  <div className="flex items-center gap-3">
                    <a
                      ...
                      className="hidden sm:inline-flex items-center gap-1 text-[11px] font-bold text-[#123B70] hover:underline px-2.5 py-1 rounded-lg hover:bg-slate-100"
                    >
                      <span>Preview Form</span>
                      <ExternalLink className="w-3 h-3" />
                    </a>

                    <div className="p-1 rounded-lg text-slate-400 bg-slate-50 border border-slate-200">
                ...
                {isExpanded && (
                  <div className="p-5 sm:p-6 border-t border-slate-100 bg-slate-50/40 space-y-6">
                    ...
                    {service.id === 'visiting-cards' && (
                      <div className="space-y-4">
                        <div>
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 bg-white p-4 rounded-xl border border-slate-200">
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-slate-200">
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-xs font-bold text-[#123B70] hover:underline flex items-center gap-1.5"
                      >
                      ...
                      <button
                        onClick={saveConfig}
                        disabled={saving}
                        className="inline-flex items-center gap-1.5 px-4 py-2 bg-[#123B70] text-white rounded-xl hover:bg-[#0c274c] transition-colors text-xs font-bold shadow-xs cursor-pointer disabled:opacity-50"
                      >
