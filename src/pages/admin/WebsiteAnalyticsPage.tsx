    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-center space-x-4">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-900">{data?.activeProducts || 0}</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-center space-x-4">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-900">{data?.inactiveProducts || 0}</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-center space-x-4">
          ...
          <div>
            <p className="text-sm font-medium text-slate-500">Missing Images</p>
            <h3 className="text-2xl font-bold text-slate-900">{data?.missingImages.length || 0}</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-center space-x-4">
          ...
          <div>
            <p className="text-sm font-medium text-slate-500">Missing Prices</p>
            <h3 className="text-2xl font-bold text-slate-900">{data?.missingPrices.length || 0}</h3>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 text-xs uppercase tracking-wider text-slate-500 border-b border-slate-100">
                  ...
                  <th className="px-6 py-3 font-medium text-right">Orders Count</th>
                </tr>
              </thead>
                ...
                {data?.topServices.length === 0 ? (
                  ...
                  data?.topServices.map((item, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-600 text-right">{item.count}</td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="p-6 border-b border-slate-100">
            <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
              <Package className="w-5 h-5 text-[#123B70]" />
              ...
              <tbody className="divide-y divide-slate-100">
                {data?.recentlyUpdated.map((product) => (
                  <tr key={product.id} className="hover:bg-slate-50/50">
                    <td className="px-6 py-4 text-sm font-medium text-slate-900">{product.name_en || product.name_hi}</td>
            ...
            <div>
              ...
              <ul className="space-y-2 max-h-60 overflow-y-auto pr-2">
                {data?.missingImages.slice(0, 20).map(p => (
                  <li key={p.id} className="text-sm bg-slate-50 p-2 rounded-lg text-slate-700 border border-slate-100">
                    {p.name_en || p.name_hi}
                  </li>
                ))}
                {data?.missingImages && data.missingImages.length > 20 && (
                  <li className="text-xs text-center text-slate-500 pt-2">...and {data.missingImages.length - 20} more</li>
                )}
                {data?.missingImages.length === 0 && <li className="text-sm text-slate-500">All products have images!</li>}
              </ul>
            </div>
            <div>
              <h4 className="text-sm font-medium text-slate-700 mb-3 flex items-center gap-2">
              ...
              <ul className="space-y-2 max-h-60 overflow-y-auto pr-2">
                ...
                {data?.missingPrices && data.missingPrices.length > 20 && (
                  <li className="text-xs text-center text-slate-500 pt-2">...and {data.missingPrices.length - 20} more</li>
                )}
                {data?.missingPrices.length === 0 && <li className="text-sm text-slate-500">All products have valid prices!</li>}
              </ul>
            </div>
          </div>
        </div>
