...
export default function IdCardProjectPage() {
  ...
  if (state.kind === 'error') {
    return (
      <div className="mx-auto mt-8 max-w-md rounded-xl border border-red-200 bg-red-50/60 p-6 text-center shadow-xs">
        <p className="text-sm font-semibold text-red-800">{state.message}</p>
        <button
          onClick={load}
          className="mt-3 inline-flex items-center justify-center rounded-lg border border-red-300 bg-white px-4 py-2 text-xs font-semibold text-red-700 hover:bg-red-50 cursor-pointer"
        >
          Retry
        </button>
      </div>
