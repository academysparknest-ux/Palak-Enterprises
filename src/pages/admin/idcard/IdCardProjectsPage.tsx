      ...
      <div className="mt-6">
        {state.kind === 'loading' && (
          <div className="flex h-40 items-center justify-center text-slate-400">
            <Loader2 className="mr-2 animate-spin text-[#123B70]" size={18} /> Loading projects...
          </div>
        )}

        {state.kind === 'error' && (
          <div className="rounded-xl border border-red-200 bg-red-50/60 p-6 text-center shadow-xs">
            ...
            <div className="mt-4 flex items-center justify-center gap-3">
              <button
                onClick={handleRetry}
                disabled={isRetrying}
                className="inline-flex items-center gap-1.5 rounded-lg border border-red-300 bg-white px-4 py-2 text-xs font-semibold text-red-700 hover:bg-red-50 cursor-pointer disabled:opacity-50"
              >
              ...
              {state.code === 'AUTH_REQUIRED' && (
                <Link
                  to="/login?returnTo=/admin/id-cards"
                  className="inline-flex items-center gap-1.5 rounded-lg bg-[#123B70] px-4 py-2 text-xs font-semibold text-white hover:bg-[#0f2d57]"
                >
